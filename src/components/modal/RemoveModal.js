...
export default function(prop) {
    ...
    async function removeCategory() {
        ...
        if (label == 'a') { 
            ...
            if (res.data().products.length > 0) {
                const msg = "This category includes products. Delete all products in order to delete category"
                setMsg(msg)
            }
            else if (res.data().products.length == 0) {
                await deleteDoc(doc(db, "categories", id))
                setMsg("")
                category.dispatch({type: 'cancel'})
            }
        }
    ...
    return (
        <div className="flex overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center bg-gray-500/75 w-full md:inset-0 h-screen max-h-full">
            <div className="relative p-4 w-full max-w-md max-h-full">
                <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div className="p-4 md:p-5 text-center">
                        ...
                        <button onClick={removeCategory} type="button" className="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center me-2">
                            Yes, I'm sure
                        </button>
            ...
            {msg && <AuthModal msg={msg} closeModal={() => {setMsg(""); category.dispatch({type: 'cancel'})}} />}
        </div>
    )
}